...
\begin{frame}
  ...
  \begin{itemize}
    \item Understand basic structure of a LaTeX document, its various document
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Any content after \lstinline{\appendix}, will be added 
    to the appendix.
...
\begin{frame}[fragile]
  ...
  \begin{center}
    Write a LaTeX script that creates a document of type article, which
     contains both a table of content and an appendix. 
    The table of content should be at the beginning of the document 
    and the appendix at the end.
    The book should contain two chapters, with the first chapter containing
     two numbered and two un-numbered sections. The first un-numbered 
    section should be present in the table of content.
  \end{center}
\end{frame}
    ...
    \begin{verbatim}
      \documentclass{article}
         ...
         \section*{Un-numbered Section3}
           ...
           First un-numbered Section Text.\\This appears in the 
        table of content
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Gained an understanding of the basic structure of a LaTeX document,
    ...
    \item Created both numbered and non-numbered sections and subsections
    in a LaTeX document.
...
\begin{frame}[fragile]
  ...
  {\footnotesize 
  \begin{enumerate}
    \item  Is the LaTeX code given below a valid input file (File compiles 
successfully and produces the intended result, that is to produce a book 
    with two chapters and an appendix.
  {\tiny 
    \begin{center}
      \begin{verbatim}
        \documentclass{book}
    ...
    \item subsection command can be placed at any arbitrary level. 
    If they get numbered by default using the appropriate setcounter 
command and secnumdepth parameter, do they automatically appear in the 
table of content ?? 
  \end{enumerate}
  }
\end{frame}
...
\label{sec-10}
\begin{enumerate}
\item Although the given file looks syntactically valid, the output file is 
    not what we expected. This is mainly because we are trying to use the 
    section command to create sections in the appendix, for a document whose 
    type is given as a book.
\vspace{15pt}
\item No, the \textbackslash tableofcontents  command normally shows only 
    numbered section headings, and only down to the level defined by the 
    tocdepth counter.
\end{enumerate}
\end{frame}
